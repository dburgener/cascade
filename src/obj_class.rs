...
const COMMON_X_DEVICE_PERMS: &[&str] = &[
    "getattr",
    "setattr",
    "use",
    "read",
...
pub fn make_classlist() -> ClassList<'static> {
    ...
    classlist.add_class(
        "x_application_data",
        vec!["paste", "paste_after_confirm", "copy"],
    );
    classlist.add_class("x_client", vec!["destroy", "getattr", "setattr", "manage"]);
    classlist.add_class(
        "x_colormap",
        vec![
            "create",
    ...
    // The x_device object class was removed from x windows in 2009:
    // https://github.com/freedesktop/xorg-xserver/commit/6c2ae5fec552366e11ad64a27626eb5dec4becf0
    // As a result, it is not included in the default object classes
    ...
    classlist.add_class("x_event", vec!["send", "receive"]);
    classlist.add_class("x_extension", vec!["query", "use"]);
    classlist.add_class(
        "x_font",
        vec![
            "create",
    ...
    classlist.add_class(
        "x_gc",
        vec!["create", "destroy", "getattr", "setattr", "use"],
    );
    classlist.add_class("x_keyboard", COMMON_X_DEVICE_PERMS.to_vec());
    classlist.add_class("x_pointer", COMMON_X_DEVICE_PERMS.to_vec());
    classlist.add_class(
        "x_property",
        vec![
            "create", "destroy", "read", "write", "append", "getattr", "setattr",
        ],
    );
    classlist.add_class("x_resource", vec!["read", "write"]);
    classlist.add_class(
        "x_screen",
        vec![
            "getattr",
    ...
    classlist.add_class("x_selection", vec!["read", "write", "getattr", "setattr"]);
    classlist.add_class(
        "x_server",
        vec!["getattr", "setattr", "record", "debug", "grab", "manage"],
    );
