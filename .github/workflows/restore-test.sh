...
# with -fcommon
sudo DESTDIR=~/selinux_out LIBDIR=~/selinux_out/usr/lib LIBSEPOLA=~/selinux_out/usr/lib/libsepol.a LDFLAGS="-L~/selinux_out/usr/lib -L~/selinux_out/usr/lib" make -j16 CFLAGS="-Wno-error=stringop-truncation -fcommon -pipe -fPIC -I~/selinux_out/usr/include" -C libsepol install
sudo ls ~/selinux_out
