...
# with -fcommon
sudo DESTDIR=~/selinux make -j16 CFLAGS="-Wno-error=stringop-truncation -fcommon -pipe -fPIC" -C libsepol install
sudo DESTDIR=~/selinux PREFIX=/usr/local make -j16 -C secilc install
