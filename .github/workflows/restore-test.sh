...
# with -fcommon
sudo DESTDIR=~/selinux LIBDIR=$(DESTDIR)/usr/lib LIBSEPOLA=$(LIBDIR)/libsepol.a CFLAGS+= -I$(DESTDIR)/usr/include LDFLAGS+= -L$(DESTDIR)/usr/lib -L$(LIBDIR) make -j16 CFLAGS="-Wno-error=stringop-truncation -fcommon -pipe -fPIC" -C libsepol install
sudo DESTDIR=~/selinux LIBDIR=$(DESTDIR)/usr/lib LIBSEPOLA=$(LIBDIR)/libsepol.a CFLAGS+="-I$(DESTDIR)/usr/include" LDFLAGS+="-L$(DESTDIR)/usr/lib -L$(LIBDIR)" make -j16 -C secilc install
